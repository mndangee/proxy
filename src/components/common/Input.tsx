...
export default function Input({ size = "medium", ...args }: IInputProps) {
  ...
  return (
    <div
      className={`${isFocus ? "outline-border-primary/70 outline-2" : ""} ${props.error ? "!outline-border-negative" : ""} ${props.disabled ? "!outline-border-week placeholder:text-label-disabled" : "hover:outline-border-primary/70"} rounded-4 outline-border-enabled flex w-full items-center gap-3 overflow-hidden outline outline-1`}
