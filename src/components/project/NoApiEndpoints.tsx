...
interface NoApiEndpointsProps {
  onRegisterApi?: () => void;
}

export default function NoApiEndpoints({ onRegisterApi }: NoApiEndpointsProps) {
  return (
    <div className="bg-background-white rounded-5 mt-9 flex w-full max-w-[1600px] flex-col items-center border-1 border-gray-200 py-14 text-center">
      ...
      <Btn category="primary" size="medium" startIcon={<PlusIcon />} onClick={() => onRegisterApi?.()} width={240}>
        API 등록하기
      </Btn>
    </div>
  );
